    ...
    def parse(self, user_id: uuid.UUID, stmt_id: int, file_path: str):
        ...
        with SessionLocal() as db:
            for item in raw_data:
                ...
                amount, formatted_date, norm_desc = self.extract_transaction_fields(item)
                if amount is None:
                    continue
            ...
            if uncat_desc_set:
                ...
                logging.info(f"[{self.__class__.__name__}] saved new global rules: {new_global_rules}")
            ...
            logging.info(f"[{self.__class__.__name__}] user_id={user_id}, parsed {len(transactions)} transactions")
